...
class BLRPRx:
    ...
    def sample_storm(
        self, theta: list, simulation_hours: int, start_time: datetime
    ) -> list:
        """
        ...
        nu = (
            alpha / theta[3]
        )  # scale parameter for gamma distribution, thetas[3]=alpha/nu
        ...
        n_cells_list = [
            1 + poisson.rvs(betas[s] * duration_storm_hrs[s]) for s in range(n_storms)
        ]

        # generate each storm's start time and calculate each storm's end time
        sDT_storm_hrs = [
            start_time + timedelta(hours=(simulation_hours * uniform.rvs()))
            for s in range(n_storms)
        ]
        eDT_storm_hrs = [
            sDT_storm_hrs[s] + timedelta(hours=duration_storm_hrs[s])
            for s in range(n_storms)
        ]
        ...
        for s in range(n_storms):
            ...
            cell_sDTs = [
                sDT_storm_hrs[s]
                + timedelta(hours=(duration_storm_hrs[s] * uniform.rvs()))
                for c in range(n_cells_list[s])
            ]
            cell_sDTs[0] = sDT_storm_hrs[s]
            cell_eDTs = [
                cell_sDTs[c] + timedelta(hours=duration_cell_hrs[c])
                for c in range(n_cells_list[s])
            ]
            ...
            cell_Depths = [
                self.RCI_model.sample_intensity(para_xis[c])
                for c in range(n_cells_list[s])
            ]
    ...
    def kernal(self, k, x, nu, alpha):
        alpha = (
            (alpha + 0.00000001)
            if ((alpha <= 4.0) and (math.modf(alpha) == 0.0))
            else alpha
        )
    ...
    def Cov(self, theta, lag=1):
        ...
        cov_p1 = (f1 + kappa * phi / (phi2 - 1.0)) * (
            self.kernal(k, h * (lag - 1.0), nu, alpha)
            - 2.0 * self.kernal(k, lag * h, nu, alpha)
            + self.kernal(k, h * (lag + 1.0), nu, alpha)
        )

        cov_p2 = (kappa / phi2 / (phi2 - 1.0)) * (
            self.kernal(k, phi * (lag - 1.0) * h, nu, alpha)
            - 2.0 * self.kernal(k, phi * lag * h, nu, alpha)
            + self.kernal(k, phi * (lag + 1.0) * h, nu, alpha)
        )
    ...
    def Mom3(self, theta):
        ...
        m3_p0 = (
            (1.0 + 2.0 * phi + phi2)
            * (phi4 - 2.0 * phi3 - 3.0 * phi2 + 8.0 * phi - 4.0)
            * (phi3)
        )
        m3_p1 = self.kernal(k1, h, nu, alpha) * (
            12.0 * phi7 * kappa2
            - 24.0 * f1 * phi2 * kappa
            - 18.0 * phi4 * kappa2
            + 24.0 * f1 * phi3 * kappa
        ...
        m3_p2 = self.kernal(k2, h, nu, alpha) * (
            24.0 * f1 * phi4 * h * kappa
            + 6.0 * phi9 * h * f2
            - 30.0 * f1 * phi6 * h * kappa
            + 6.0 * f1 * phi8 * h * kappa
            + 54.0 * phi5 * h * f2
            - 24.0 * h * f2 * phi3
            - 36.0 * phi7 * h * f2
        )
        m3_p3 = self.kernal(k1, phi * h, nu, alpha) * (
            -48.0 * kappa2
            + 6.0 * f1 * phi4 * kappa
            - 48.0 * phi * f1 * kappa
            + 6.0 * phi5 * kappa2
        ...
        m3_p4 = (
            self.kernal(k2, phi * h, nu, alpha)
            * (h * kappa2)
            * (-24.0 * phi + 30.0 * phi3 - 6.0 * phi5)
        )
        m3_p5 = self.kernal(k1, 0.0, nu, alpha) * (
            72.0 * phi7 * f2
            + 48.0 * phi * f1 * kappa
            + 24.0 * f1 * phi2 * kappa
            - 36.0 * f1 * phi3 * kappa
        ...
        m3_p7 = self.kernal(k1, 2.0 * h, nu, alpha) * (
            -12.0 * f1 * phi4 * kappa
            - 3.0 * f1 * phi8 * kappa
            + 15.0 * f1 * phi6 * kappa
            - 3.0 * phi7 * kappa2
        ...
        m3_p8 = self.kernal(k1, h * (1.0 + phi), nu, alpha) * (
            -24.0 * f1 * phi3 * kappa
            - 6.0 * f1 * phi4 * kappa
            + 6.0 * f1 * phi5 * kappa
            + 24.0 * f1 * phi2 * kappa
        ...
        return (the_lambda * muc * iota * iota * iota / m3_p0) * (
            m3_p1 + m3_p2 + m3_p3 + m3_p4 + m3_p5 + m3_p6 + m3_p7 + m3_p8
        )
    ...
    def Ph(self, theta):
        ...
        ph_tmp = (-1.0 * the_lambda) * (h + mut) + the_lambda * Gp * (
            phi + kappa * math.pow((nu / (nu + (kappa + phi) * h)), (alpha - 1.0))
        ) / (phi + kappa)
        ph = 0.0
