...
PARAM_R = 3.44428647676

# gsl_rng_min, gsl_rng_max
min = 0
max = 65535

# tabulated values for the heigt of the Ziggurat levels
ytab = [1, 0.963598623011, 0.936280813353, 0.913041104253,
...
ktab = [0, 12590644, 14272653, 14988939,
...
wtab = [1.62318314817e-08, 2.16291505214e-08, 2.54246305087e-08, 2.84579525938e-08,
...
def ziggurat():
    ...
    def UNI(jsr):
        return 0.5 + SHR3(jsr) * 0.2328306e-9

    return

# this function is yet to be completed
def gaussian_ziggurat(sigma):
    offset = min
    range_num = max - offset

    while True:
        if range_num > 0xFFFFFFFF:
            k = get() - offset
            i = (k & 0xFF)
            j = (k >> 8) & 0xFFFFFF

        elif range_num >= 0x00FFFFFF:
            k1 = get() - offset
            k2 = get() - offset
            i = (k1 & 0xFF)
            j = (k2 & 0x00FFFFFF)
            ...
            i = uniform_int(256) # choose the step
            j = uniform_int(16777216) # sample from 2^24
        ...
        if (i < 127):
            ...
            y = expon.rvs(-PARAM_R * (x - 0.5 * PARAM_R)) * U2

        if y < expon.rvs(-0.5 * x * x):
            break

    return sign * sigma * x

def test_gamma(alpha, nu):
    if alpha > 1:
        return 0, gamma.rvs(alpha, nu)
    u = uniform.rvs()
    while u < 0.1 or u > 0.9999999999:
        u = uniform.rvs()
    return u, gamma.rvs(1+alpha, nu, size=1)[0] * (u ** (1/alpha))
