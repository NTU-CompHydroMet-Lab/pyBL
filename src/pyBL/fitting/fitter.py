...
from pyBL.models.blrprx import _blrprx_covariance, _blrprx_mean, _blrprx_variance, _blrprx_moment_3rd
        ...
        def evaluation_func(x: npt.NDArray[np.float64]) -> np.float64:
            ...
            for t, scale in enumerate(scales):
                ...
                if mask_np[t,0]:     # Stat_Props.MEAN.value
                    predict[t, 0] = mean
                    score += np.power(predict[t, 0] - target_np[t, 0], 2) * weight_np[t, 0]
                if mask_np[t,1]:   # Stat_Props.CVAR.value
                    predict[t, 1] = np.sqrt(variance) / mean
                    score += np.power(predict[t, 1] - target_np[t, 1], 2) * weight_np[t, 1]
                if mask_np[t,2]:   # Stat_Props.SKEWNESS.value
                    predict[t, 2] = moment_3rd / np.power(variance, 1.5)
                    score += np.power(predict[t, 2] - target_np[t, 2], 2) * weight_np[t, 2]
                if mask_np[t,3]:   # Stat_Props.AR1.value
                    ...
                    score += np.power(predict[t, 3] - target_np[t, 3], 2) * weight_np[t, 3]
                if mask_np[t,4]:   # Stat_Props.AR2.value
                    ...
                    score += np.power(predict[t, 4] - target_np[t, 4], 2) * weight_np[t, 4]
                if mask_np[t,5]:   # Stat_Props.AR3.value
                    ...
                    score += np.power(predict[t, 5] - target_np[t, 5], 2) * weight_np[t, 5]
                if mask_np[t,6]:   # Stat_Props.pDRY.value
                    predict[t, 6] = 0
                    score += np.power(predict[t, 6] - target_np[t, 6], 2) * weight_np[t, 6]
                if mask_np[t,7]:   # Stat_Props.MSIT.value
                    predict[t, 7] = 0
                    score += np.power(predict[t, 7] - target_np[t, 7], 2) * weight_np[t, 7]
                if mask_np[t,8]:   # Stat_Props.MSD.value
                    predict[t, 8] = 0
                    score += np.power(predict[t, 8] - target_np[t, 8], 2) * weight_np[t, 8]
                if mask_np[t,9]:   # Stat_Props.MCIT.value
                    predict[t, 9] = 0
                    score += np.power(predict[t, 9] - target_np[t, 9], 2) * weight_np[t, 9]
                if mask_np[t,10]:   # Stat_Props.MCD.value
                    predict[t, 10] = 0
                    score += np.power(predict[t, 10] - target_np[t, 10], 2) * weight_np[t, 10]
                if mask_np[t,11]:   # Stat_Props.MCS.value
                    predict[t, 11] = 0
                    score += np.power(predict[t, 11] - target_np[t, 11], 2) * weight_np[t, 11]
                if mask_np[t,12]:   # Stat_Props.MPC.value
                    predict[t, 12] = 0
                    score += np.power(predict[t, 12] - target_np[t, 12], 2) * weight_np[t, 12]
                if mask_np[t,13]:   # Stat_Props.VAR.value
                    predict[t, 13] = variance
                    score += np.power(predict[t, 13] - target_np[t, 13], 2) * weight_np[t, 13]
                if mask_np[t,14]:   # Stat_Props.AC1.value
                    ...
                    score += np.power(predict[t, 14] - target_np[t, 14], 2) * weight_np[t, 14]
                if mask_np[t,15]:   # Stat_Props.AC2.value
                    ...
                    score += np.power(predict[t, 15] - target_np[t, 15], 2) * weight_np[t, 15]
                if mask_np[t,16]:   # Stat_Props.AC3.value
                    ...
                    score += np.power(predict[t, 16] - target_np[t, 16], 2) * weight_np[t, 16]
            #return np.nansum(np.power(predict - target_np, 2) * weight_np * mask_np)
            return score
        return evaluation_func

    @staticmethod
    def default_target(timescale: List[float] = [1, 3, 6, 24]):
        # Create a pandas dataframe with Stat_Props as columns one row with timescales=1,3,6,24
        target_df = pd.DataFrame(columns=[prop for prop in Stat_Props], index = timescale, dtype=np.float64)
        target_df.index.name = "timescales"

        return target_df

    @staticmethod
    def default_weight(timescale: List[float] = [1, 3, 6, 24]):
        # Create a pandas dataframe with Stat_Props as columns one row with timescales=1,3,6,24
        weight_df = pd.DataFrame(columns=[prop for prop in Stat_Props], index = timescale, dtype=np.float64)
        weight_df.index.name = "timescales"
        return weight_df
    
    @staticmethod
    def default_mask(timescale: List[float] = [1, 3, 6, 24]):
        ...
        mask_df = pd.DataFrame(data=mask_np, columns=[prop for prop in Stat_Props], index = timescale, dtype=np.int_)
