...
def compare_plt(output, timeseries, fin_df, prop, scalelt):
    for scale in scalelt:
        ...
        bp = ax.boxplot(
            data, sym="", whis=[0, 100], widths=0.4, labels=labels, patch_artist=False
        )
        ...
        delta_means = [
            abs(sample_mean_findf[i] - test_mean[i])
            for i in range(len(sample_mean_findf))
        ]
        ...
        #     )
        plt.scatter(
            range(1, len(test_stat.index[:]) + 1),
            test_stat.loc[:, stat_prop],
            label="obs",
            color="blue",
            marker="x",
        )
        plt.plot(
            range(1, len(fin_df.index[:]) + 1),
            fin_df.loc[:, stat_prop],
            label="sample",
            color="green",
            marker="o",
        )
        ...
        ax.grid()
        plt.savefig(
            "./02 Output_data/Sampling_result/"
            + output
            + "/"
            + output
            + "_"
